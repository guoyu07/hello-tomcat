...
* Run the app
    ```
    $ ./gradlew clean build
    $ SPRING_PROFILES_ACTIVE=default,db,development java -jar build/libs/hello-tomcat-0.0.1-SNAPSHOT-all.jar "http://localhost:8888"
    ```
* Hit the URL
...
* If you want to fetch encrypted values then you have to set `USE_ENCRYPT` to `true`.
	...
	```
	$ SPRING_PROFILES_ACTIVE=default,db,development USE_ENCRYPT=true java -jar build/libs/hello-tomcat-0.0.1-SNAPSHOT-all.jar "http://localhost:8888"
	```
